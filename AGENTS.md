...
- **Stock Edge validation 纠偏（2026-05-08）**：`pit8` / `top30` / 几天 PIT replay 只能作为 smoke，不得作为参数、算法、horizon edge 或 YAML 晋升证据；但也不要从 smoke 直接跳到五年全量硬跑。专业节奏改为：第一阶段先跑最近约 6 个月交易日的 PIT-local / stratified universe 半年级验证，验证 pipeline、参数方向、5/10/20d horizon 稳定性与样本覆盖；第二阶段做多个 6 个月窗口交叉（recent 6m、prior 6m、2022 bear/range、2023/2024/2025 不同 regime 窗口），区分 recent OOS 与 historical regime robustness；最后才考虑 2021-2026 walk-forward / purged CV 作为最终 YAML 固化 gate。五年全量不一定更好，旧市场结构可能导致过拟合或稀释当前 edge。
- **Stock Edge objective 对齐修正（2026-05-08）**：Stock Edge 调参/验证目标必须直接服务“未来 5/10/20 个交易日选股收益最大化”。`panel_evaluator` 已输出并纳入 objective 的核心项包括 `rank_ic_quality`、`top_bucket_return_quality`、`top_bottom_spread_quality`、`bucket_monotonicity_quality`、`top_bucket_win_quality`、`top_bucket_left_tail_penalty`；`objectives.py` 已修正为真正读取 YAML `tuning.objective.horizon_weights`。`pit8` / `topN` 小样本只能 smoke，不得作为 YAML 固化证据。
- **Stock Edge 3 个月正式 stratified PIT 验证（Codex 2026-05-08）**：基于 `9e63388` 后续工程，新增 replay panel chunk split、`--pit-lookback-days`、outcome-first evaluator/report。已跑 recent 3m `top=100`、18 个 PIT 日期 `2026-01-08`→`2026-03-30`、stratified-pit、K=3 walk-forward、two-stage search；panel 1791/1800 成功，失败率 `0.50%`，缓存 `/Users/neoclaw/claude/ifaenv/data/stock/replay_panels/top_liquidity_top100_stratifiedpit__20260108_20260330__c339deb1c055.parquet`，outcome artifact `/Users/neoclaw/claude/ifaenv/manifests/stock_edge_panel_diagnostics/top_liquidity_top100_stratifiedpit__20260108_20260330__c339deb1c055__outcome_20260508.json`。K-fold validation tuned lift：5d `[-0.074,+0.007,-0.021]` positive 1/3；10d `[+0.083,-0.016,-0.018]` positive 1/3；20d `[+0.044,-0.144,-0.239]` positive 1/3。Candidate full-panel headline：5d avg `-0.58%`, rank IC `+0.049`, top bucket `+0.14%`, spread `+1.39%`; 10d avg `-0.62%`, rank IC `+0.083`, top bucket `+1.04%`, spread `+2.38%`; 20d avg `-0.76%`, rank IC `+0.056`, top bucket `-1.12%`, spread `+1.37%`。月份切片显示 2026-03 明显失效（5d/10d/20d rank IC `-0.028/-0.121/-0.095`，top bucket 均负），行业切片中食品饮料/商贸零售/轻工/地产 10d 显著负。结论：验证 pipeline 完成，但参数不过 gate；未改/未固化 YAML。下一步 6m/多窗口前必须优先做 chunk-level checkpoint/resume 和更快 replay/label cache，否则 100×18 已需约 72 分钟。
- **Stock Edge validation infra checkpoint/proxy（Codex 2026-05-08）**：基于 `eb59a1b` 后续工程，`build_replay_panel()` 新增 sidecar chunk checkpoint/resume，不改变最终 panel cache key；每个 chunk 完成后写 `<panel_stem>.chunks/<chunk>.parquet/json`，中断重跑会跳过已完成 chunk，`checkpoint_manifest.json` 与最终 manifest 记录 requested/completed/failed、chunk_size、runtime、failure_details。新增 `ifa.families.stock.backtest.outcome_proxy` 与 `scripts/stock_edge_panel_tune.py --proxy-only`，先批量生成 forward 5/10/20d labels + cheap PIT features（动量、波动、成交额、moneyflow、市值、行业、regime）和 outcome diagnostics，避免 6m/多窗口前盲目跑 full strategy replay。小样本 smoke：`--as-of 2026-04-30 --top 5 --pit-samples 2 --pit-lookback-days 120 --universe-mode stratified-pit --proxy-only --dry-run` 成功，10/10 行、proxy build 1.5s、未跑 full replay/search、未改 YAML。
- **Stock Edge sector_cycle_leader redesign foundation（Codex 2026-05-08，2026-05-09 补充）**：按用户新方向新增 research-only proxy family `sector_cycle_leader`，路径是 sector-first 再 leader-within-sector。`outcome_proxy_v2` 会在批量 proxy panel 中读取 SME stock/sector orderflow、sector diffusion/state/risk flags，生成主力累积、散户拥挤、热度确认、leader quality、tradability 等 PIT-safe 特征，再进入 5/10/20d Rank IC/top bucket 诊断。新增 `stock.theme_heat_weekly` Alembic schema 与 `scripts/stock_edge_theme_heat_stub.py`，当前只是 weekly compatibility cache，占位/缓存周级摘要，不作为最终形态或 alpha 证据。用户最新要求已明确：theme heat 不能只是每周 top5 静态表，必须扩展为 multi-resolution rolling theme heat，包括 raw news/announcement/event rows 按 endpoint/source watermark 增量入库，1h/2h/4h intraday snapshots（源频率支持时），daily heat curve（`heat_level`、`heat_delta`、`heat_acceleration`、`persistence_days`、sector/theme breadth、main-money lead vs retail chase alignment、crowding/distribution risk），weekly top themes + cached LLM summary；LLM 不是保守 stub，正确定位是用 repo 内 `ifa.core.llm.LLMClient` 做周度/日度高阶主题遴选、归类、解释、A 股板块/股票映射、持续性与风险判断；不能逐条新闻/per-stock 实时打标签。事实输入优先来自本地缓存 Tushare 新闻/公告/事件、Research rows、TA catalyst rows；事实不足时允许 LLM prior 初筛，但必须 `quality_flag=llm_prior_only` 或 `needs_local_evidence`，不得当强 alpha。已新增 `scripts/stock_edge_theme_heat_llm.py` 作为正式 daily/weekly batch 入口：daily `--persist` 先写本地 JSON artifact，weekly `--persist` 写 `stock.theme_heat_weekly`；`--dry-run` 只输出 prompt/schema/fact pack，`--from-json` 支持审核后离线 ingestion。weekly 兼容 path 仍保留 `--build-llm` / `--llm-dry-run`，prompt/schema 版本 `stock_theme_heat_llm_weekly_v1`，daily 版本 `stock_theme_heat_llm_daily_v1`，扩展字段写入 `stock.theme_heat_weekly.evidence_json` 或 daily artifact。sector-cycle-leader 后续应结合 theme heat curve 与主力/散户资金曲线判断板块周期阶段。设计文档见 `docs/stock_edge_sector_cycle_leader_design.md`。生产 YAML 不得因该 proxy family 自动变更，必须先走 6m 60-PIT/top200 proxy、60-PIT full replay subset、多窗口/最终 walk-forward gate。
- **Stock Edge recommendation brief 边界纠偏（Codex 2026-05-09）**：`Stock Edge recommendation brief` 现在明确等于 sector-cycle/leader 专项推荐简报，只使用上一已收盘交易日的板块资金/热度周期、主力 vs 散户结构、拥挤/出货风险、板块内龙头/承接、5/10/20 日窗口适配、风险和失效条件。不得把 TA、Ningbo 或 Research diagnostic candidates 混入推荐池、打分、fallback 或证据列表；它们只能作为独立报告，未来最多做交叉引用。模板、页眉页脚、disclaimer 继续复用 iFA report style。
- **Stock Edge recommendation brief P0 实现审计（Codex 2026-05-09）**：已补 `stock theme_heat_daily` Alembic 日度热度曲线表，daily LLM theme heat `--persist` 现在同时写 JSON artifact 与 `stock.theme_heat_daily`，字段覆盖 `heat_level`、`heat_delta`、`heat_acceleration`、`persistence_days`、sector/stock breadth、main-money vs retail alignment、crowding/distribution risk、horizon evidence。`stock edge recommendation-brief` 继续只读 `stock.sector_cycle_leader_daily` + `stock.risk_veto_daily` + `stock.theme_heat_daily` + SME evidence，输出 strong/watchlist/avoid；无 TA/Ningbo/Research fallback，不改 production YAML，不 auto-promote，不改 cron/delivery。
- **Stock Edge sector-cycle score normalization（Codex 2026-05-09）**：`sector_cycle_leader_v1` builder 修正 `top5_main_net_share` 未 clamp 导致 `sector_score/leader_score` 可大于 1 的问题；新逻辑把 top5 concentration clamp 到 `[0,1]` 后进入 sector score。已重跑 `2026-05-06`，`stock.sector_cycle_leader_daily` 5,180 行更新，recommendation brief smoke 输出分数恢复到 0-1 区间。风险 flags 同步过滤 `None/null` 空值，避免客户简报出现 `"None"`。
- **Stock Edge 6 个月验证口径最终设置（Codex 2026-05-08）**：此前“6 个月必须覆盖全部约 120 个交易日、不能 60 PIT”的解释已被用户最终纠偏覆盖。当前明确设置为 `lookback=180 calendar days`、`PIT dates=60 个交易日`、`top=200 stocks/date`，proxy-only 先跑约 12,000 rows；full replay 阶段可缩 cross-section subset，但时间 PIT 仍保持 60。仍然不改 production YAML，不 `auto-promote`，不 `apply-to-baseline`。
- **Stock Edge 6 个月 proxy-only 最终复核（Codex 2026-05-08）**：用户最终明确验证标准为 `lookback=180 calendar days`、`PIT dates=60 个交易日`、`top=200 stocks/date`、先跑 `proxy-only` 约 12,000 rows；此前任何 pit8/pit36 结果都只是 smoke/不足样本，报告必须承认不足，不得作为参数或 YAML 固化证据。本轮已按用户指定标准运行：`uv run python scripts/stock_edge_panel_tune.py --as-of 2026-05-06 --pit-lookback-days 180 --pit-samples 60 --top 200 --universe-mode stratified-pit --proxy-only --force-proxy-cache`。PIT dates `2025-12-25`→`2026-03-30`，60/60 个交易日，每日 200 只，12,000 requested / 11,976 ok / failure `0.20%`，universe batch 选择约 `18.8s`，proxy build 约 `30.2s`。proxy cache `/Users/neoclaw/claude/ifaenv/data/stock/outcome_proxy/top_liquidity_top200_stratifiedpit__20251225_20260330__3711e32196b2.parquet`，最新 diagnostics `/Users/neoclaw/claude/ifaenv/manifests/stock_edge_panel_diagnostics/top_liquidity_top200_stratifiedpit__outcome_proxy_20260508_173920.json`。候选 family 结果：`weak_industry_avoid_quality_flow` overall rank IC 5/10/20d `+0.055/+0.061/+0.101`，top bucket return `-0.05/-0.16/-0.44%`，top win `0.462/0.430/0.406`，spread `+0.71/+1.15/+2.25pp`；2026-03 rank IC `+0.054/+0.025/+0.036`，top return `-1.72/-2.80/-2.32%`，10/20d spread `-0.15/-0.36pp`。`industry_relative_momentum_flow` overall rank IC `+0.042/+0.054/+0.079`，top bucket return `-0.02/+0.00/-0.35%`，top win `0.453/0.429/0.394`，spread `+0.42/+1.24/+1.80pp`；2026-03 rank IC `+0.062/+0.059/+0.070`，top return `-1.50/-2.21/-0.83%`，spread `+0.60/+1.37/+1.58pp`。结论：两个 family 都有正排序 edge，`industry_relative_momentum_flow` 在 2026-03 明显更稳；但绝对 top-bucket return 仍弱，不足以改 YAML。下一步 full replay gate 可以缩 cross-section subset，但时间 PIT 必须仍保持 60；建议用 `--proxy-family-gate` + checkpoint/resume，加入 baseline/random control，不 auto-promote、不 apply-to-baseline。
