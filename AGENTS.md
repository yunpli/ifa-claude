...
- **Stock Edge 6 个月 proxy-only + 性能优化（Codex 2026-05-08）**：按用户明确规格跑完 recent 6m `--as-of 2026-05-06 --pit-lookback-days 180 --pit-samples 60 --top 200 --universe-mode stratified-pit --proxy-only --force-proxy-cache`，PIT dates `2025-12-25`→`2026-03-30`，12,000 requested / 11,976 ok / failure `0.20%`，proxy cache `/Users/neoclaw/claude/ifaenv/data/stock/outcome_proxy/top_liquidity_top200_stratifiedpit__20251225_20260330__3711e32196b2.parquet`，diagnostics `/Users/neoclaw/claude/ifaenv/manifests/stock_edge_panel_diagnostics/top_liquidity_top200_stratifiedpit__outcome_proxy_20260508_173434.json`。性能改动：stratified PIT universe selection 从逐 PIT SQL 改成窗口批量拉取 daily/basic/SW membership 后 pandas PIT 分层；outcome proxy 从逐 PIT date 4 组 SQL 改成窗口批量拉取 daily/moneyflow/basic/member/regime 后内存组装；manifest 写入 query/index/assemble timing；proxy-only 报告直接包含 candidate family comparison。benchmark：universe 3-date old/new `17.335s → 3.300s` (`5.25x`，counts 一致)；proxy 3-date old/new `14.210s → 6.399s`，按旧 per-date loop 外推 60-date 约 `284s` vs full batch `53.8s`，约 `5.3x`；完整 universe selection `30.5s`，完整 proxy build `53.8s`。候选 family 结果：`weak_industry_avoid_quality_flow` overall rank IC 5/10/20d `+0.055/+0.061/+0.101`，top spread `+0.71/+1.15/+2.25pp`，但 2026-03 top avg `-1.72/-2.80/-2.32%` 且 10/20d spread略负；`industry_relative_momentum_flow` overall rank IC `+0.042/+0.054/+0.079`，top spread `+0.42/+1.24/+1.80pp`，2026-03 rank IC `+0.062/+0.059/+0.070`、spread `+0.60/+1.37/+1.58pp`，相对更稳但 top avg仍为负。结论：proxy family 有排序 edge，尤其相对 baseline cheap composite 在 2026-03 明显改善；但绝对 top-bucket return 仍弱，不足以改 YAML。下一步 full replay gate 应只做缩小 cross-section：60 PIT 保持不变，每 family 每 PIT 取 top 4-8，加 baseline/random control，约 480-1,000 rows，走 `--proxy-family-gate` + checkpoint/resume，不 auto-promote、不 apply-to-baseline。
