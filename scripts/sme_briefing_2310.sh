...
# Third-party scheduler entry for the recommended Beijing evening SME briefing.
# Run after the 22:40 incremental ETL has finished. The observed report date is
# the same Beijing trading date, which avoids early-morning "previous day"
# ambiguity. Non-trading days return structured JSON and exit 0.
...
GATE_ARGS=(uv run python scripts/sme_daily_gate.py --kind brief --brief-target same-day --json)
...
{
  echo "SME evening briefing started_at_bjt=$(TZ=Asia/Shanghai date)"
  echo "root=${ROOT_DIR}"
  echo "log=${LOG_FILE}"
