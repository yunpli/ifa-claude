"""Macro text-derived capture job — entry point."""
...
from ifa.jobs.common.news_source import SOURCES_FOR_TEXT_CAPTURE
from ifa.jobs.common.runner import JobReport, run_scan_and_extract
from ifa.jobs.common.text_filter import MACRO_INDICATOR_SPECS

from . import prompts
from .repo import upsert_indicator_rows

JOB_NAME = "macro_text_derived_capture"


def run_macro_text_capture(
    *,
    ...
    def _process(batch: list[CandidateInput], result: BatchResult) -> int:
        n = 0
        # parsed entries are aligned by index with the input batch
        for cand in batch:
            entry = next(
    ...
    return run_scan_and_extract(
        job_name=JOB_NAME,
        sources=SOURCES_FOR_TEXT_CAPTURE,
        keyword_specs=MACRO_INDICATOR_SPECS,
...
def _label_from_src(cand: CandidateInput) -> str:
    # Reconstruct source_label as `{api}.{src}` for repo's source_table column
    # using simple heuristics: npr's `src` is usually empty / 国务院类
    if cand.source.startswith("国务院") or cand.source in {"国务院", "国务院办公厅"}:
        return "npr"
    if cand.source in {"sina", "wallstreetcn", "10jqka", "eastmoney", "cls", "yicai", "jinrongjie", "fenghuang", "yuncaijing"}:
