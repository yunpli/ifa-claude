...
生成"今日宏观底色"判断卡，面向 A 股投资人。这是顾问开篇，必须具备**前瞻指引**价值，**不能与下面 §02-§07 的宏观面板/流动性/政策矩阵重复**。
...
5. **top3 必须 3 条**，每条 ≤ 22 个汉字。每一条都是**今天盘中要做的一件事**：盯哪条宏观信号通过什么 A 股表现得到验证、哪个事件可能改变底色。**不能写成"PPI同比0.5%"这种 §02 已经展示的事实**。
6. bullets：3-5 条关键判断（支撑底色的材料），每条 dimension（政策/增长/通胀/流动性/汇率/跨资产/科技政策）+ judgment（≤14 字）+ a_share_implication（≤20 字对 A 股含义）+ data_timing（最近一期已披露/上一交易日确认/隔夜至cutoff/政策记忆）+ confidence（high/medium/low）。
7. 不写"建议买入/卖出"等指令性语言。
8. 如果某些变量数据不足，明确说明，不要硬解读。"""
