# V2.2 / V2.2.2 Schema 综合 Review

> **目的**：在三家族开始 Alembic migration 前，统一 review schema 设计的一致性、外键完整性、命名规范、索引覆盖。
> **范围**：`research.*`（11 张表）/ `ta.*`（13 张表）/ `stock.*`（8 张表 PG + 4 张 DuckDB）/ V2.2.2 `sme.*`（资金结构派生、标签、调参、报告表）
> **既有依赖**：`smartmoney.*`（约 20 张已有表）、`ningbo.*`（约 10 张已有表）、`public.*`（report_runs 等核心表）
> **V2.2.2 更新**：SME 作为独立 family 加入 schema 体系；代码不依赖 SmartMoney，只只读消费 `smartmoney.*` 源表，所有派生产物写入 `sme.*`。
...
erDiagram
    ...
    %% ─── SME V2.2.2 独立资金结构家族 ───────────────────────
    SMARTMONEY_RAW_MONEYFLOW ||--o{ SME_STOCK_ORDERFLOW : "read-only ts_code,trade_date"
    SMARTMONEY_SW_MEMBER_MONTHLY ||--o{ SME_SW_MEMBER_DAILY : "PIT membership"
    SME_SW_MEMBER_DAILY ||--o{ SME_SECTOR_ORDERFLOW : "l2_code,trade_date"
    SME_STOCK_ORDERFLOW ||--o{ SME_SECTOR_ORDERFLOW : "aggregate"
    SME_SECTOR_ORDERFLOW ||--o{ SME_SECTOR_DIFFUSION : "rolling trading days"
    SME_SECTOR_DIFFUSION ||--o{ SME_SECTOR_STATE : "state machine"
    SME_SECTOR_ORDERFLOW ||--o{ SME_LABELS : "forward trading-day labels"
    SME_SECTOR_STATE ||--o{ SME_MARKET_STRUCTURE : "daily buckets"
    SME_MARKET_STRUCTURE ||--o{ SME_STRATEGY_EVAL : "bucket × labels"
```
