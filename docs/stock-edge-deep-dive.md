...
> - **§11.4** 工程闸门(idempotency cache / compute lock / model version A-B / audit log / drift monitor)
...
**必加表 `stock_edge.universe_history`**:
...
**必加 `stock_edge.calibration_metrics`** 周度任务:
...
此外构建独立 setup `O4_NORTHBOUND_ACCUMULATION` (见 §5.7)。
...
闭环验证必须采集真实 fill 价。新增表 `stock_edge.fill_log`:
...
🕐 数据新鲜度: 5/4 15:00 ✓ | TA 5/4 ✓ | Research Q1 (30d) ⚠
...
新增表 `stock_edge.analysis_lock` (文档 §11.1 已提,这里展开):
...
#### 11.4.3 Model Version + A/B Switching
...
models:
  right_tail_classifier:
    production_version: v1.2  
    candidate_version: v1.3   # 上线 24 周开始 forward eval
    ab_split: 0.10             # 10% 流量走 candidate
...
新增表 `stock_edge.param_change_log`:
...
新增表 `stock_edge.calibration_metrics` (周度任务):
...
- `stock_edge.universe_history` PIT 表(含退市股)
- `stock_edge.calibration_metrics` 周度任务
...
8. P(+50%, 40d) 和收益分位来自 ML / analog / 统计集成
...
26. Model version + A/B switching 框架就绪
