...
**实现要点**：
- Stock Intel 读 Research 用 SQL 直接查 `research.report_sections` 最近成功 deep 的 content_json
- 不复制数据，每次按需读
...
```bash
# 速览（最快）
ifa research quick --company 智微智能

# 标准（多季对比）
ifa research standard --company 001339 --start 2023-01-01

# 深度（完整尽调）
ifa research deep --company 智微智能 --with-pdf --output-pdf

# 用代码或简称都行
ifa research deep --company 中电电机
ifa research deep --company 603988

# 指定输出
ifa research deep --company 测绘股份 --output-dir ~/Desktop/

# 控制缓存
ifa research deep --company 智微智能 --no-cache
ifa research deep --company 智微智能 --refresh-data
ifa research deep --company 智微智能 --rerun-report

# 健康检查（只跑数据完备性矩阵，不出报告）
ifa research check --company 智微智能
```

### 7.2 HTTP API（供 Telegram bot 复用）
