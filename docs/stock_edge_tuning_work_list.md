...
| **T1.2** | G5 Bootstrap CI gate | 半天 | 每 horizon val rank IC 做 1000 次 bootstrap（np.random sampling with replacement）；新增 G5 gate：95% CI 下界 > 0；vectorized；artifact metrics 暴露 `bootstrap_ci` 字段；CLI 默认开 G5 | none | ✅ done (commit pending) |
