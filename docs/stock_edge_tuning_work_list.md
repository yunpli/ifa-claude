...
| **T3.4** | 决策 ledger DB 表 + git tag 自动化 | 半天 | 新表 `stock.tuning_promotion_log`：列 (timestamp, artifact_path, gates_passed_json, variant_yaml_path, git_tag, applied_to_baseline)；每次 auto_promote_if_passing 写一行；git tag 命名 `stock-edge-tune-YYYYMMDD-HHMMSS` | T1 完成 | ✅ done (commit pending) |
