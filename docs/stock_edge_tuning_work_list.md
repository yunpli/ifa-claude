...
|---|---|---|---|---|---|
| **T1.1** | K-fold 一致性 gate | 半天 | `evaluate_promotion_gates` 接受 `kfold_results: list[fold_metrics]`；新增 G9 gate：要求 ≥`min_positive_folds`(default 3/4) folds 在每 horizon 上 lift > 0；CLI `--k-fold-min-positive` 参数；K-fold 路径下默认调用 G9 | none | ✅ done (commit pending) |
| **T1.2** | G5 Bootstrap CI gate | 半天 | 每 horizon val rank IC 做 1000 次 bootstrap（np.random sampling with replacement）；新增 G5 gate：95% CI 下界 > 0；vectorized；artifact metrics 暴露 `bootstrap_ci` 字段；CLI 默认开 G5 | none | ⏳ todo |
| **T1.3** | G4 Regime-bucketed gate | 半天 | 按 `ta.regime_daily` 把 val rows 分桶（panel 已有 regime 字段）；每桶 ≥30 样本时算 rank IC；新增 G4 gate：要求 ≥75% 桶改善（lift > 0）；artifact metrics 暴露 `regime_breakdown` | none | ⏳ todo |
| **T1.4** | Horizon-selective promotion | 半天 | `auto_promote_if_passing` 接受 `per_horizon_decisions: dict`；只把 pass 的 horizon 的 weights 写到 variant YAML；其他 horizon 保留 baseline；CLI 报告每 horizon 的 promotion 决定（applied / kept_baseline） | T1.1 | ⏳ todo |
...
| ID | 任务 | 估时 | Acceptance Criteria | 依赖 | Status |
|---|---|---|---|---|---|
| **T2.1** | Optuna TPE 取代随机搜索 | 半天 | 引入 `optuna` 依赖；`fit_global_preset_via_panel` 加 `search_algo` 参数（`'random' | 'tpe'`）；TPE 模式下用 IC priors 作为 enqueue 的 first trial；K-fold 下每 fold 独立 study；CLI `--search-algo tpe` | none | ⏳ todo |
| **T2.2** | Successive halving | 半天 | 三阶段：广搜 N → top 25% 进精搜 N/2 → top 5% 进 fine N/4；CLI `--search-stages a,b,c`；artifact 暴露每阶段最优 | T2.1（基于 TPE）| ⏳ todo |
| **T2.3** | G3 Sharpe-like gate | 半天 | 计算 `avg_return / max_drawdown` per horizon；新增 G3 gate：tuned ratio ≥ baseline ratio - 0.05（容差 5%）；artifact 暴露 sharpe-like ratio | none | ⏳ todo |
| **T2.4** | G8 收敛 stat 形式化 | 1 小时 | search_history 已存 per-iter best；新增 G8 gate：最近 3 轮 best 的 std/mean < 5%；artifact 暴露 convergence_stat | none | ⏳ todo |
...
| ID | 任务 | 估时 | Acceptance Criteria | 依赖 | Status |
|---|---|---|---|---|---|
| **T3.1** | DB I/O batching（per-as_of pre-load） | 1 天 | 主进程对每个 as_of 一次性 SELECT all-stocks 的 daily / moneyflow / sector flow / regime；通过 multiprocessing `initializer` 把字典传给 worker；worker 内 gateway 优先读 in-memory cache | none | ⏳ todo |
...
| **T3.4** | 决策 ledger DB 表 + git tag 自动化 | 半天 | 新表 `stock.tuning_promotion_log`：列 (timestamp, artifact_path, gates_passed_json, variant_yaml_path, git_tag, applied_to_baseline)；每次 auto_promote_if_passing 写一行；git tag 命名 `stock-edge-tune-YYYYMMDD-HHMMSS` | T1 完成 | ⏳ todo |
