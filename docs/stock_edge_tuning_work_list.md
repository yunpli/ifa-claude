...
| Tier | Tasks | 完成 |
|---|---|---|
| Tier 1 | T1.1 / T1.2 / T1.3 / T1.4 | ✅ 4/4 |
| Tier 2 | T2.1 / T2.2 / T2.3 / T2.4 | ✅ 4/4 |
| Tier 3 | T3.1 ✅ / T3.4 ✅ / T3.2 ⏳ / T3.3 ⏳ | ✅ 2/4 |

10/12 tasks done. 余下 T3.2 (ML 跨日期复用，2 天复杂工作) + T3.3 (大 panel 终验，依赖 T3.2 完整 perf)。

### 0.3 当前实测（2026-05-05，K=4 folds × 50 stocks × 12 dates 16-month panel）
...
| ID | 任务 | 估时 | Acceptance Criteria | 依赖 | Status |
|---|---|---|---|---|---|
| **T3.1** | DB I/O batching（per-as_of pre-load） | 1 天 | 主进程对每个 as_of 一次性 SELECT all-stocks 的 daily / moneyflow / sector flow / regime；通过 multiprocessing `initializer` 把字典传给 worker；worker 内 gateway 优先读 in-memory cache | none | ✅ done (`167573e`, in-worker preload) |
