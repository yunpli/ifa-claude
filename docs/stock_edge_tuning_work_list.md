...
|---|---|---|---|---|---|
| **T1.1** | K-fold 一致性 gate | 半天 | `evaluate_promotion_gates` 接受 `kfold_results: list[fold_metrics]`；新增 G9 gate：要求 ≥`min_positive_folds`(default 3/4) folds 在每 horizon 上 lift > 0；CLI `--k-fold-min-positive` 参数；K-fold 路径下默认调用 G9 | T1.4 setup | ⏳ todo |
