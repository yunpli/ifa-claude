...
| **T1.3** | G4 Regime-bucketed gate | 半天 | 按 `ta.regime_daily` 把 val rows 分桶（panel 已有 regime 字段）；每桶 ≥30 样本时算 rank IC；新增 G4 gate：要求 ≥75% 桶改善（lift > 0）；artifact metrics 暴露 `regime_breakdown` | none | ✅ done (commit pending) |
