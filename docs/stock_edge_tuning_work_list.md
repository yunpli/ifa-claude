...
| **T1.4** | Horizon-selective promotion | 半天 | `auto_promote_if_passing` 接受 `per_horizon_decisions: dict`；只把 pass 的 horizon 的 weights 写到 variant YAML；其他 horizon 保留 baseline；CLI 报告每 horizon 的 promotion 决定（applied / kept_baseline） | T1.1 | ✅ done (commit pending) |
