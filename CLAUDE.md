...
- **本 session（TA family）**：负责 `ifa/families/ta/**` + `tests/ta/**` + 相关 alembic 迁移 + `docs/ta-strategy-deep-dive.md` / `docs/scoring-principles.md` / `docs/family-reference.md` 中的 TA 段。
- **另一 session（research family UI）**：负责 `ifa/families/research/**`（含 `report/` 下的模板与样式） + `tests/research/**` + `docs/research-deep-dive.md`。
- **共享但谨慎**：`ifa/core/**`（DB / units / 渲染基础）、`alembic/versions/`（不同迁移文件互不冲突，但 head 推进时注意 down_revision）、根目录 `CLAUDE.md`（按各自段落各自维护）。
